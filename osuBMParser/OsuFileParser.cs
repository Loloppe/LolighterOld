    ...
    public class OsuFileParser
    {
        ...
        internal void parse(OsuFileSection sections)
        {
            ...
            foreach (string line in lines)
            {
                ...
                if (!string.IsNullOrWhiteSpace(line))
                {
                    ...
                    {
                        Console.WriteLine("Parsing" + currentSection.ToString());
        ...
        private void hitObjectParse(string data)
        {
            ...
            if (hitObject is HoldNote)
            {
                ...
                {
                    string endtime = Regex.Match(tokens[5], @"\d+").Value; //We only need the first int
                    ((HoldNote)hitObject).EndTime = toInt(endtime);
                }
            }
